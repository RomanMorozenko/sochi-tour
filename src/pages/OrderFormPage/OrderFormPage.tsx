...
import {
  ...
  ToursCollectionResponseItemType
...
export const OrderFormPage = () => {
  ...
  let targetItem: ToursCollectionResponseItemType | ExcursionCollectionResponseItemType =
    {} as ToursCollectionResponseItemType;
  if (section === 'tours') {
    const item = tours.find((tour: ToursCollectionResponseItemType) => tour.id === id);
    if (item) targetItem = item;
  } else {
    const item = excursions.find(
      (excursion: ExcursionCollectionResponseItemType) => excursion.id === id
    );
    if (item) targetItem = item;
  }
